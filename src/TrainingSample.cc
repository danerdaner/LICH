...
TrainingSample aTrainingSample ;
TrainingSample::TrainingSample()
	: Processor("TrainingSample"),
	_output(0)
{
...
int TrainingSample::SubDeFlag(TVector3 inputPos){
...
int TrainingSample::ActiveLayers(std::vector<CalorimeterHit*> clu, const std::string& encoder_str){
...
int TrainingSample::NHScaleV2( const std::string& encoder_str, std::vector<CalorimeterHit*> clu0, int RatioX, int RatioY, int RatioZ )
{
...
float TrainingSample::FDV2(std::vector<CalorimeterHit*> clu, const std::string& encoder_str)
{
	float FractalDim = 0;
...
float TrainingSample::DisSeedSurface( TVector3 SeedPos )
{
...
TVector3 TrainingSample::ClusterCoG(Cluster * inputCluster)
{
		...
		for(int i=0;i<11;i++){
			...
			for(int j=0;j<4;j++){
				...
				TString spos=inputDetectorModules[j].c_str();
		   		wgtFname = weightDir + "/TMVA_"+slevel+"GeV_"+spos+"/weights/TMVAMulticlass_"+slevel+"GeV_"+spos+"_BDTG.weights.xml";
				TMVA::Tools::Instance();
...
void TrainingSample::NewClusterFlag(Cluster* a_tree, Track* a_trk)
{
...
void TrainingSample::SamplePro( LCEvent * evtP )
{
	if (evtP)
	{
...
void TrainingSample::processEvent( LCEvent * evtP )
{
	...
	if (evtP)
	{
		...
		TrainingSample::gearPara();	
		TrainingSample::SamplePro( evtP );	
	}
}

void TrainingSample::end()
{
