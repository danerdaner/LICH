...
//   TCut mycuts = "NTrk==1 && MCPP[2]/MCPEn<0.7"; // for example: TCut mycuts = "abs(var1)<0.5 && abs(var2-0.5)<1";
   ...
   factory->AddTree( treePi, TString("Pi"), 1. ,"abs(cosTheta)>0.7&&abs(cosTheta)<0.8&&NPFO==1");
   factory->AddTree( treeMu, TString("Mu"), 1.,"abs(cosTheta)>0.7&&abs(cosTheta)<0.8&&NPFO==1");
//   factory->AddTree( treeE, TString("E"), 1., mycuts);
   factory->AddTree( treeE, TString("E"), 1.,"abs(cosTheta)>0.7&&abs(cosTheta)<0.8&&NPFO==1");
